...
await (await tangle.setMinBal(100, { gasPrice: 0 })).wait()
assert.equal('100', (await tangle.minBal()).toString(), 'minBal not correctly set')

let B_wallet = Wallet.createRandom().connect(provider)
tangle = tangle.connect(B_wallet)
assert.equal(await tangle.setMinBal(50, { gasPrice: 0 }).catch((reason: any) => { return reason }) instanceof Error, true, 'unauthorized minBal set did not fail')
